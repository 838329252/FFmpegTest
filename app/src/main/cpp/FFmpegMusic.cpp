...
/*static void (*audio_call)(uint8_t *out_buffer,int size);*/
void *MusicPlay(void *args){
    LOGE("ffmpegmusic:执行MusicPlay方法");
...
int getPcm(FFmpegMusic *agrs){
    ...
    while (agrs->isPlay){
        ...
        int ret=avcodec_send_packet(agrs->codec,avPacket);
        if(ret<0){
            LOGE("ffmpegmusic:解码错误");
            continue;
        }
...
void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context){
    //得到pcm数据
    LOGE("ffmpegmusic:回调pcm数据")
    FFmpegMusic *musicplay = (FFmpegMusic *) context;
    int datasize = getPcm(musicplay);
    if(datasize>0){
        //第一针所需要时间采样字节/采样率*通道数*采样位数（转化为字节数）
        double time = (double)datasize/((double)44100*2*2);
        //
        musicplay->clock=time+musicplay->clock;
        LOGE("ffmpegmusic:当前一帧声音时间%f  播放时间%f",time,musicplay->clock);
...
int createFFmpeg(FFmpegMusic *agrs){
    LOGE("ffmpegmusic:初始化SwrContext")
